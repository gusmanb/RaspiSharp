...
namespace Dalssoft.DiagramNet
{
	...
	public class Document: IDeserializationCallback 
	{
		...
		public Document(Designer Designer)
		{
		...
		public void AddElement(BaseElement el)
		{
			...
			el.OnAddedToDocument(new DocumentEventArgs { Document = this });
		}
		...
		public void DeleteElement(BaseElement el)
		{
			if ((el != null) && !(el is ConnectorElement))
			{
				...
				elements.Remove(el);
				el.OnRemovedFromDocument(new DocumentEventArgs { Document = this });
				OnAppearancePropertyChanged(new EventArgs());	
...
//			ControlPaint.DrawGrid(g, clippingRegion, gridSize, Color.LightGray);
			...
			var offset = designer.DocumentOffset;
			...
			size.Height = (int)Math.Max(size.Height, windowSize.Height / zoom);
			size.Width = (int)Math.Max(size.Width, windowSize.Width / zoom);

			for(int buc = 0; buc < size.Height; buc += gridSize.Height)
				g.DrawLine(p, 0, buc, size.Width, buc);

			for (int buc = 0; buc < size.Width; buc += gridSize.Width)
				g.DrawLine(p, buc, 0, buc, size.Height);
			/*
			int xInterval = (int)(gridSize.Width * zoom);
			int yInterval = (int)(gridSize.Height * zoom);

			int minX = ((int)(designer.DocumentOffset.Height / gridSize.Height)) * gridSize.Height;
			int 
